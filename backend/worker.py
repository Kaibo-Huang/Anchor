...
def analyze_videos_task(self, event_id: str):
    ...
    2. Index all videos (compressing if needed for TwelveLabs 2GB limit)
    ...
    print(f"[Worker:analyze_videos] ========== STARTING ANALYSIS ==========")
    ...
    try:
        ...
        # Prepare all videos for parallel processing
        print(f"[Worker:analyze_videos] ---------- PREPARING VIDEOS ----------")
        video_tasks = []
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            for i, video in enumerate(videos.data):
                ...
                # Download video to check size and potentially compress
                local_video_path = os.path.join(tmpdir, f"video_{video['id']}.mp4")
                download_file(bucket, key, local_video_path)

                file_size = os.path.getsize(local_video_path)
                file_size_gb = file_size / (1024 * 1024 * 1024)
                print(f"[Worker:analyze_videos] Video {i + 1} size: {file_size_gb:.2f} GB")
                ...
                if file_size > MAX_SIZE_BYTES:
                    ...
                    compressed_url = upload_file(compressed_path, bucket, compressed_key, "video/mp4")
            ...
            print(f"[Worker:analyze_videos] All {len(pending_tasks)} videos submitted, waiting for completion...")

            # Wait for all tasks to complete
            print(f"[Worker:analyze_videos] ---------- WAITING FOR INDEXING ----------")
            ...
            for pt in pending_tasks:
                if pt["task_id"]:
                    ...
                    print(f"[Worker:analyze_videos] Video {pt['index'] + 1} indexed: {completed_task.video_id if hasattr(completed_task, 'video_id') else 'N/A'}")
            ...
            def generate_embeddings_for_video(ct):
                print(f"[Worker:analyze_videos] Generating embeddings for video {ct['index'] + 1}...")
...
def generate_video_task(self, event_id: str):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Download videos
            print(f"[Worker:generate_video] ---------- DOWNLOADING VIDEOS ----------")
            video_paths = []
            for i, video in enumerate(videos.data):
                print(f"[Worker:generate_video] Downloading video {i + 1}/{len(videos.data)}: {video['id']}")
                bucket, key = parse_s3_uri(video["original_url"])
                local_path = os.path.join(tmpdir, f"{video['id']}.mp4")
                download_file(bucket, key, local_path)
                file_size = os.path.getsize(local_path) / (1024 * 1024)  # MB
                print(f"[Worker:generate_video] Downloaded: {file_size:.1f} MB")
                video_paths.append({
                    "id": video["id"],
                    "path": local_path,
            ...
            print(f"[Worker:generate_video] All videos downloaded")
...
def generate_highlight_reel_task(self, event_id: str, reel_id: str, query: str, vibe: VibeType, duration: int = 30):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print(f"[Worker:highlight_reel] Downloading source videos...")
            clip_paths = []
            for i, clip in enumerate(selected_clips):
                print(f"[Worker:highlight_reel] Downloading clip {i + 1}/{len(selected_clips)}")
